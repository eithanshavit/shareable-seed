...
function validateVersionName (versionName) {
  if (!versionCodes.hasOwnProperty(versionName)) throw new Error(INVALID_SHAREABLE_CODE_VERSION)
  return true
}

function validateVersionHex (versionHex) {
  if (!_.find(versionCodes, function (v, k) { return v === versionHex })) throw new Error(INVALID_SHAREABLE_CODE_VERSION)
  return true
}
