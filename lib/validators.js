...
function validateMnemonic (mnemonic) {
  ...
  if (mnemonicList.count < 12 || mnemonicList.count > 24) throw new Error(errors.INVALID_MNEMONIC)
...
function validateThreshold (threshold) {
  ...
  if (threshold < 2 || threshold >= 2 ** 8) throw new Error(errors.INVALID_THRESHOLD)
  return true
}
...
function validateVersionHex (versionHex) {
  if (!_.find(versionCodes, function (v, k) { return v === versionHex })) throw new Error(errors.INVALID_SHAREABLE_CODE_VERSION)
  return true
}
...
function validateShareableCodeChecksum (shareableCode) {
  ...
  if (givenChecksum !== computedChecksum) throw new Error(errors.INVALID_SHAREABLE_CODE_CHECKSUM)
  return true
}
